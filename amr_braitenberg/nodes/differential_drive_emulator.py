...
class DifferentialDriveEmulatorNode:
	...
	def _wheel_speed_callback(self, msg):
		if len(msg.speeds)!=2:
			...
			twist = Twist()
			#extracting the left and right velocities from \cmd_vel_diff topic
			vel_left = msg.speeds[0]
			vel_right = msg.speeds[1]
			#calculating the linear and angular velocities from vel_left and vel_right
			linear_v = (vel_right + vel_left)*self._wheel_diameter
			angular_v = ((vel_right - vel_left)*self._wheel_diameter)/(self._distance_between_wheels/2)
			#setting up the linear and angular values in the twist message
