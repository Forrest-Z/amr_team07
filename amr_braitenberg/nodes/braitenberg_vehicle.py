...
<<<<<<< HEAD
		...
		def __init__(self):
				...
				self._sonar_subscriber = rospy.Subscriber('/sonar_braitenberg',Ranges, self._sonar_callback, queue_size=100)
				self._wheel_speeds_publisher = rospy.Publisher('/cmd_vel_diff',WheelSpeeds, queue_size=100)
				self._dynamic_reconfigure = dynamic_reconfigure.server.Server(BraitenbergVehicleConfig, self._reconfigure_callback)
		...
		def _sonar_callback(self, ranges_msg):
				...
				"""
				ws = WheelSpeeds()
				#taking the reading of the sonar sensors into a variable
				self.sensor = ranges_msg
				#extracting the readings of left and right sensors separately
				left_reading = self.sensor.ranges[0].range/self.sensor.ranges[0].max_range
				right_reading = self.sensor.ranges[1].range/self.sensor.ranges[0].max_range
				#creating an onject of WheelSpeeds class

				#assigning the speeds by sending the readings to compute_wheel_speeds function
				ws.speeds = self._vehicle.compute_wheel_speeds(left_reading,right_reading)
				#Publishing the speeds to the cmd_vel_diff topic
				self._wheel_speeds_publisher.publish(ws)

				# Output the debug info:
				rospy.logdebug('[{:.2f}, {:.2f}] --> [{:.2f}, {:.2f}]'.format(ranges_msg.ranges[0].range, ranges_msg.ranges[1].range, ws.speeds[0], ws.speeds[1]))
		...
		def _reconfigure_callback(self, config, params):
				...
				rospy.logdebug('Vehicle reconfigured: type {}, '
											 'factors {:.2f}] and {:.2f}]'.format(['A','B','C'][config.type], config.factor1, config.factor2))
				return config
